	...
	function mobileView(maxWidth) {
		if (maxWidth.matches) {
			var menu_icon = "<a class='menu_header' id='menu'><svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\">\n" +
				...
				"          </svg>" +
				"       </a>";
			var div = "<div id='header'> </div>"
			$("#bodyContent").before(div);
			$("#header").append(menu_icon);
			$("#header").append($("#firstHeading"));

			var menu = $("#menu");
			var main = $("#content, #mw-navigation");
			var drawer = $("#drilldown-tables-tabs-wrapper");
			var formatTabsWrapper = $('#drilldown-format-tabs-wrapper');
			if (drawer.length != 0) {
				$("body").prepend(drawer);
			} else {
			...
			if (mapCanvas.length) {
				...
				var zoom = 1 - (mapWidth / 700);
				mapCanvas.css('zoom', zoom);
			}

			var tableTabsHeader = $('#tableTabsHeader');
			var formatTabsHeader = $('#formatTabsHeader');
